  import {h,p,span,h1,h2,h3,pre,a,br,div,label,input,hr,makeDOMDriver
  } from '@cycle/dom';
  ...
  function main(sources) {
    ...
    var factorsAction9$ = factorsClick9$.map(() => {
      ...
      var i = 0;
      while (i < 25) {
        bnD(145)(x => x * x * x)(it4_c)(it6_c)(it7_c);
        i += 1;
      }
    });
...
h('div', {
  ...
  h('br'),
  h('div', {
    style: { fontSize: "32px", color: "#FFD700", textAlign: "center"
    }
  }, 'THE ESSENCE OF FUNCTIONAL JAVASCRIPT'),
  h('br'),

]), h('div.content', [ // 2 brackets  main -> content ->
h('br'),

h('p', ' The phrase "functional programming" in the context of JavaScript is often understood to mean more than preferring to accomplish goals with functions rather than explicit prototypal inheritance or pseudo classes. Writing functional JavaScript code is thought to require all functions to be pure and all values to be immutable. ' ),
h('p', ' Of course we rarely want functions to change the environments in which functions in other parts of a program operate. Communication between parts of programs is best confined to the scopes of functions. But the ability to mutate values inside the scopes of functions is a nice feature of JavaScript. Here\'s a trivial but delightful (to me, anyway) example: ' ),
...
h('span', ' and its safer counterpart, ' ),
h('a', {props: {href: "#bind"}}, 'makeBind()'),
h('span', '. As is apparent from the definition of Bind(), '),
h('br'),
h('pre', `    var f = Bind(\'f\');  ` ), 
h('span', ' creates a global variable "f" with a value similar to _bind() and a globally accessible array named "Bind.f".  '),
h('p', ' Demonstration 2 features the function "makeBind()". Running ' ), 
  
h('pre', `    var o = makeBind(\'o\');  ` ), 
h('span', ' initializes a variable "o" that references an object. "o.b" is a copy of _bind() and "o.a" is some number such that Bind[o.a] is the array that fills with returned values (or resolution values of Promises) when you run ' ),
h('pre', `    o.b(function1, function2, ... functionN)` ),
h('span', '._You can compose with a completely anonymous function like this:  '),
h('pre', {style: {color: " #AADDAA",fontSize: "17px"}}, `  
    makeBind().b(function1)(function2) ... (functionN)(). 
  `),
...
h('br'),
h('br'),
h('div', styleFunc(["rgb(255,255,100", , "26px", , , "center"]), 'Functions Returned by Bind()'),
h('div', styleFunc(["lightBlue", , "19px", "italic", , "center"]), 'And Methods (functions in objects) Returned by makeBind()'),
h('br'),
h('p', ' Let\s look at a function "bind" created by running "const bind = Bind(\'bindo\')": '),
...
h('pre', `  const bind = Bind("bind0");
     // "bind" is a reserved work, hence "bind0".
  ...
  const factorsAction7$ = factorsClick7$.map( e => {
      ...
      while (i < 25) {
          ...
          i += 1;
            ...
            h('p', ' The good news is that the results displayed in this example can be achieved with safe, anonymous copies of _bind0. Example 3 has the details. Demonstration 1 is here to ease the reader into an understanding of Bind() and the function it returns: "_bind()". '),
            h('br'),
          ...
          h('div', {
            ...
            h('p', ' The statement "f = Bind(\'f\')") returns a fresh copy of _bind but not of the array Bind["f"]. After f() has been created and called, subsequent calls push additional data into Bind[\'f\']. You can\'t get reliable information from an array containing entries pushed into it by concurrent calls to f().  '),
  ...
  . . .  `),
            h('p', ' The user generates "n". _B1, _B2, ... , _B9 are ensconsed in the virtual DOM. test4() is defined on the right. -> '),
              ...
              h('br'),
              h('p', ' This is the code behind the above results: '),
  ...
  const test6 = a => w => {
      ...
      (x=>idP(x+Math.floor(Bind[a][0]*Bind[a][1]*(Bind[a][2]/Bind[a][3]))))()
            ...
            h('div', {
              ...
              h('p', ' This demonstration doesn\'t need an array of return values, so the abbreviated version of Bind() named "Bd" is being used. As always, I encourage readers to modify the functions I present and to write original higher-order functions to suit their needs. Here are "Bd", "bnD", and "factorsction9$": '),
              h('pre', `  function Bd () {
      var p = [];
  ...
  var factorsAction9$ = factorsClick9$.map(() => { 
      ...
      while (i < 25) {
          ...
          i += 1; 
            ...
            h('div', styleFunc(["#361B01", , , , "90%", "center"]), '**************************************************************************************************************'),
            h('h3', styleFunc(["#8ffc95", , "23px", , , "center"]), ' Demonstration 4 - Using _bind() With Primative Values '),
            ...
            h('div', {
              ...
              h('span.tao', ' There are, however, advantages to letting data accumulate. In this demonstration, qfB() is defined by "qfB = makeBind()". qfB.b() is called three times before Bind[qfB.a] is emptied. qFunc is called on the values in Bind[qfB.a] after the third one arrives, feeding them into the '),
              h('a', {
                props: {
  ...
  oneAction$ = qF1$.map(e => {
      ...
      if (!(aa === aa)) { 
          ...
          h('div.content', [ // 4
            ...
            h('p', ' bind() and similar functions are returned by Bind() and its safer counterpart makeBind(). Here are the definitions: '),
            h('pre', {
              style: {
                color: "lightBlue"
              }
  ...
  var bind9 = Bind("bind9");) `),
            ...
            }, 'Back to the preview demos'), // content2 2222222222222222
