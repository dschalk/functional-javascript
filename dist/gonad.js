var diffRender = function () {
  return 8
};; // See document.onload in maim

const makeBind = () => {
  var x = 1;
  while (Bind[x] != undefined) {
    x += 1
  };
  return {
    a: x,
...
var bindTest = function bindTest() {
  test5(-4);
...
function rett(x, id = "default") {
  return new Monad(x, id)
};
...
function bindEmitterDriver() {
  return xs.create({
    start: listener => {
      bindEmitter.on = msg => listener.next(msg)
    },
    stop: () => {
      bindEmitter.removeAllListeners()
    }
  });
};
...
var tr3 = function tr3(fibsArray, primesArray) {
  ...
  if (primesArray.slice(-1)[0] >= bound) {
    primes = primesArray.filter(function (v) {
      return v <= bound;
    });
  }
  var ar = [];
  ...
  fibs.map(function (v) {
    if (primesArray.every(function (p) {
        return (v % p || v == p);
      }))
      ar.push(v);
  });
...
var factorsMonad = new MonadState('factorsMonad', [
  [],
  [], 2, []
], factor_state);
...
function rang(n, m) {
  return Array.from(new Array(m - n), function (x, i) {
    return i + n;
  });
}

var ranOdd = n => m => {
  return Array.from(new Array((Math.floor(m / 2 + 2)) - n), function (x, i) {
    return 2 * i + n;
  });
}
...
var expand = function expand(a, b) {
  return a + ', ' + b;
};
...
var filter = function filter(x, condition) {
  var ar = x.slice();
  return ret(ar.filter(function (v) {
    return condition;
  }));
};
...
var decompMonad = new MonadState('decompMonad', [3, [
  [0],
  [1],
  [2]
], 3, [
...
function lcm(cx, dx) {
  ...
  d.map(v => {
    ...
    r = ar.concat(d).concat(c).reduce(function (a, b) {
      return a * b
    })
  })
  return r
...
var fpTransformer = function fpTransformer(fibsState, primeState, then) {
  ...
  execP(primeState, Math.ceil(Math.sqrt(fibsState[1]))).bnd(ps => {
    fibsState[3].map(fs => {
      if (ps[3].filter(r => r <= fs).every(p => (fs % p || fs == p))) {
        ar.push(fs)
      };
    })
    var now = Date.now();
    ...
    postMessage([
      [fibsState[3].join(', '), ps[2], ar.join(', '), elapsed], ps
    ])
  })
}
...
function styl(s) {
  switch (s) {
    ...
    case (1):
      ...
      break;
    case (2):
      return ['inline', 'inline', 'none', 'none'];
      break;
    case (3):
      return ['inline', 'inline', 'inline', 'none'];
      break;
    case (4):
      return ['inline', 'inline', 'inline', 'inline'];
      break;
    default:
      return; //console.log('Bad argument in styl. s is', s );
  }
}
...
var gameMonad = new MonadState('gameMonad', [
  [
    [0, 0, 0, [],
      [1, 2, 3, 4], "nobody", "solo"
    ],
...
MonadState4.prototype.clear = function () {
  this.s = []
}
