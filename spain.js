...
h('div', {style: {textAlign:"center", fontWeight: "bold"}}, [
  h('br'),  
  h('div', {style: {fontSize: "32px", color: "#FFD700",textAlign: "center"}}, 'COMPOSITION OF JAVASCRIPT FUNCTIONS'),
  h('br'),
...
h('span', '.  bind(), created by running "var bind = Bind("bind0"), is one such function. bind() is a copy of __bind() that is unique in that it maintains data in its own array, Bind.bind0. ' ),
h('p', ' In the example below, it is apparent from the definition of Bind() that bind() pushes its first argument, "x", into Bind.bind0 and returns an anonymous function that takes function1 as its argument and calls bind() on function1(x). bind() then pushes function1(x) into Bind.bind0 and returns its anonymous function which calls __bind on function2(function1(x)). ' ), 
h('p', ' Recursion continues until there are no more functions or until (), null, or some variable equal to null is encountered. A final null value prompts the anonymous function to return the array of each function\'s return value or pending Promise resolution value. In the example below, that array is named "Bind.bind0". ' ),
...
h('div', styleFunc(["#58fdb2",,"20px",,,"center"]), 'myFunc(x) = bind(x)(function1)(function2) ... (functionN)().pop()' ), 
h('br'),  
...
h('div', COW8),
h('div', COW9), */
h('div', `${Bind[a1.a].join(", ")}` ),
h('div', `${Bind[a2.a].join(", ")}` ),
h('div', `${Bind[a3.a].join(", ")}` ),
h('div', `${Bind[a4.a].join(", ")}` ),
h('div', `${Bind[a5.a].join(", ")}` ),
h('div', `${Bind[a6.a].join(", ")}` ),
h('div', `${Bind[a7.a].join(", ")}` ),
h('div', `${Bind[a8.a].join(", ")}` ),
h('div', `${Bind[a9.a].join(", ")}` ),
h('span', ' value of n --> '),
