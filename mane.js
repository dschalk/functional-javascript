...
// socket = new WebSocket(hws://204.48.16.214:3055h);
...
h('a', { props: { href: "https://cycle.js.org/h, target: h_blank" } }, 'A Cycle.js application')    ]),
 ...
 h('a', {props: {href: "#Bind3"}}, Bind" ),
 h('span', '. Syntax consists of "calling bin(x)" for some value "x" and then naming or defining the functions to be composed on "x". A \"(terminate)\" flag at the end causes an array of return values (or resolution values of Promises) to be returned. Consider:' ), 
 h('br'),
 h('br'),
 h('div', styleFunc(["#FFD700h,h3%",h21pxh,,,]), 'bind(x)(functiona1)(function2) ... (functionN)'),
         h('br'),

         h('span', styleFunc(["#d3ead5h,,h18px",,,]), 'where '),
...
h('h3', styleFunc([h#8ffc95h,h3%h,,,,]), ' Demonstration 1 - A Simple Example.'),
h('span.tao', ' If you understand why hresult === Bind.fooh (below) returns true, you have a pretty good grasp of how foo and, more generally, ' ),
h('a', {props: {href: h#bind2h }},  hBindh ),
h('span', ' work. ' ),
h('br'),
...
h('p', ' In the demonstration on the right, enter a value for hxh in runTest(x) and see the result. ' ),
h('h3', styleFunc([h#8ffc95h,h3%h,,,,]), ' Demonstration 2 - WebSocket and worker messaging.'),
...
h('p#defsReturn'),
h('span.tao', ' it4() and it6() are asynchronous functions that use hMonadItterh callback handlers in conjunction with Ecmascript 2015 promises. MonadItter is discussed in the '),
h('a', {props: {href: h#defsh }},  'appendix'),
h('span', '.' ),
h('br'),
...
h('h3', styleFunc([h#8ffc95h,h3%h,,,,]), ' Demonstration 3 - avoiding clashes.'),
h('span', ' Demonstration 2 shows the results of computations run sequentially from top to bottom in columns from left to right. Nine similar functions run orthogonally to one another thanks to the constructor hBindh. Bind.bind1, Bind.bind2, ..., Bind.bind9 are inside of nine hdivh elements in the virtual DOM. Demonstration 2 shows the arrays as they grow from empty to four-membered arrays. '),  
h('span', '. Here is the '), 
h('a', {props: {href: h#bind2h }},  hdefinition of Bindh ),
h('p', ' bind1 is a function. Bind.bind1 is an array not of the promises returned by each function, but rather it is an array of the values that are produced as each promise resolves. hBind.bind1h is a permanent fixture in the virtual DOM description returned by main(). You can watch it and nine similar arrays as their lengths seem to increase in parallel by entering an integer in the right column.'),
...
h('span.tao', {style: {color: h#FFD700h}}, `hbind1(e.target.value)(cubeP)h` ),
h('span', ' cubes user inpup. The first element of each Bind array attribute (Bind.bind1, Bind.bind2, etc) is the a number that is encapsulated in a promise. The promise resolves after 1200 ms, providing its resolution value to cubeP. When cubeP resolves, its resolution value becomes the argument of the next promise\'s then method. The delayed identity function hidPh takes the difference of the original value and its cube as its argument. After this resolves, the original value\'s cube is added to its resolution value. '),
h('p#backToDem3', '     '), 
h('h3', styleFunc([h#8ffc95h,h3%h,,,,]), ' Demonstration 4 '),
h('span#backToDem2', ' This demonstration uses a closure named hPingpongMakerh instead of Bind to define clones of the function hpingh. Three instances of ping operate independently of one another in the demonstration on the right. Here is '),
...
h('input#test7', {style: {height: h15pxh }} ),
h('br'),
h('br'),
h('div', {style: {color: h#9fe0f4h }}, [
  h('span', hrunTestResult[0]: h ),
  h('span', {style: {color: hredh}}, runTestResult[0].join(', ') ),
  h('br'),
  h('span', hrunTestResult[1] === Bind.foo: h ),
  h('span', {style: {color: hredh}}, runTestResult[1].toString() )
]),
...
h('br'),
h('br'),
h('span', styleFunc([hrgb(7, 247, 247)h,h12%h,h20pxh,,,,,]), 'bind(145)(x=>x**3)(it4)(it6)(it7)'),
h('br'),
h('br'),
h('button#factors_Q', {style: {fontSize: '15px'}}, 'bind(145)(x=>x**3)(it4)(it6)(it7)'),
h('span', h~~h),
h('button.clear_Q', {style: {fontSize: '15px', marginLeft: h0h}},  'clear results'),
h('br'),
h('br'),
h('div#demo2', m42_ ),
h('br'),
...
h('span', ' value of n --> '),
h('input#test5', {style: {height: h15pxh }} ),
h('br'),
...
h('a', {props: {href: h#curryDefh}}, hcurry and reverseCurry h ),
h('span', '. curryReverse(parseInt) can be used to define convenient functions like hbinaryToInth and htoInth. ' ),
h('pre', `  const binaryToInt = curryReverse(parseInt)(2); 
  binaryToInt(h010101h)  // 21
...
h('br'),
h('a', {props: {href: hhttps://lodash.com/docs/4.17.4#flowh}}, h  .flowh ),
h('span', ' and Ramda\'s '),
h('a', {props: {href: hhttp://ramdajs.com/docs/#composeh}}, 'R.compose' ),
...
h('a', { props: { href: hhttps://github.com/dschalk/functional-javascripth, target: h_blankh }}, 'functional-javascript'),
h('span', '. Please leave a comment in the '),
h('a', {props: {href: h#commentsh}}, 'comments'),
...
h('p', ' Visitors to this site are automatically logged in with pseudo-randomly generated numbers as their user names and passwords. The default hgrouph is the non-group hsoloh. '),
...
h('h3', 'The Game'),
h('p', 'People who are in the same group, other than the default non-group named hsoloh, share the same todo list, chat messages, and simulated dice game. '),
...
h('p', {style: {color: hredh}}, nameMess ),
...
h('br'),
h('br'),
h('div', {style: {fontSize: h14 pxh}}, 'Name: ' + pMname.x ),
h('div', {style: {fontSize: h14 pxh}}, 'Group: ' + pMgroup.x ),
h('br'),
h('div', {style: {fontSize: h14 pxh}}, gameData),
h('br'),
h('div#a100', ' _____________________________________ ' ),
h('p.italic', ' Join group hth if you want to see some previously created tasks. ' ),
...
h('p', ' bind() and similar functions are created by the constructor named hBindh. Here is Bind()\'s definition: '),
h('pre', {style: {color: hlightBlueh}}, `  function Bind (str) {
    Bind[str] = [];
    return function bindo ( x ) {
      ...
      return function debug8 (func) {
        var p;
        if (func.name === hterminateh) return Bind[str];
        if (x instanceof Promise) {
          p = x.then(v => func(v));
        }
        else p = func(x);
...
h('pre', `  var bind = Bind(hbind0h);
  var bind1 = Bind(hbind1h);
  var bind2 = Bind(hbind2h);
  var bind3 = Bind(hbind3h);
  var bind4 = Bind(hbind4h);
  var bind5 = Bind(hbind5h);
  var bind6 = Bind(hbind6h);
  var bind7 = Bind(hbind7h);
  var bind8 = Bind(hbind8h);
  var bind9 = Bind(hbind9h); ` ),
h('a', {props: {href: '#content2'}}, 'Back to the preview demos'),
h('p', italicYellow, ' Sequences beginning with bind() reveal exactly what is happening while sometimes hiding confusing conglomerations of nested functions, object methods, and promises. They are a flexible way out of hCallback Hellh. '),
h('p#cycle'),
h('a', { props: { href: '#top' } }, 'Back To The Top'),
h('br'),
h('a', {props: {href: '#cyclet'}}, 'Async Procedures' ),
h('p', ' hh(\'div\', m42_RESULT)h is a permanent fixture in the Snabbdom virtual DOM that is returned by main() and updated by calcStream$. When it7() executes, Snabbdom performs its diff and render routine, updating the browser window. '),
...
h('a', { props: { href: hhttps://egghead.io/courses/cycle-js-fundamentalsh, target: h_blankh } }, ' Overview of Cycle.js.'),
...
h('a', {props: {href: hfph}}, 'appendix' ),
h('span', '. ' ),
h('br'),
h('pre', {style: {color: hredh, fontStyle: hitalich}} , `Red Indicates An Ongoing Computation` ),
...
h('p', ' Instances of Monad can also link by means of the method hbnd()h. It, along with the method hret()h, were made available as follows: ' ),
h('pre',  {style: {color: hrgb(236, 242, 186)h   }}, `  Monad.prototype.bnd = function (func, ...args) {
...
h('pre', `  ret(2).bnd(v => add(v,1)
  ...
  h('h2', 'MonadItter'),
  h('p', ' As shown in the hMonadsh section (above), the definition of MonadItter is: ' ),
  code.monadIt,
  ...
  h('p', 'For any instance of MonadItter, say hith, hit.bnd(func)h causes it.p === func. Calling the method hit.release(...args)h causes p(...args) to run, possibly with arguments supplied by the caller. '),
  ...
  h('input#testW'),
  h('p', ' cube() is defined in the Monad section (above). If you click hmMZ1.release(1)h several times, the code (above) will run several times, each time with v === 1. The result, mMt3.x, is shown below the button. mMZ1.p (bnd()\'s argument) remains constant while mMZ1.release(1) is repeatedly called, incrementing the number being cubed each time. '),
  ...
  h('input#quad'),
  h('p', ' The function hsolve()h is at the center of the algorithm. See how mMZ3.bind() appears three times. Each time a user enters a number, say hxh, mMZ3.release(x) executes. After the third number, solve calls itself so the process can start again. I prefer this approach over ES6 generators. Here\'s solve(): '),
  code.quad,
  ...
  h('p', ' The function solve() is recursive. It invokes itself after release() executes three times. The expression hsolve()h resets solve to the top, where mMZ3.p becomes a function containing two nested occurrences of mMZ3.bnd. After mMZ3.release() executes, mMZ3.p becomes the function that is the argument to the next occurrence of mMZ3.bnd. That function contains yet another occurrence of mMZ3.bnd. MonadItter is syntactic sugar for nested callbacks. ' ),

h('h3', ' Preserve Archives By Sidestepping Mutation '),
h('p', ' When you assign a variable to an array, for example hvar arr = [1,2,3]h, arr points to a location in memory. Suppose you want to preserve a record of previous values of arr. You might try saving them in an array like this: ' ),
h('pre', `  var arr = [1,2,3]
  var b = [arr];
  arr.push(4);
  b.push(arr);
  ...
  b[0] == b[1]  // true` ),
h('p', ' That didn\'t work! b points to [arr] and arr still points to its original location in memory. harr.push(4)h mutated the value in arr\'s location and hb.push(arr)h added a copy. hb[0] == b[1]h returning true verifies that both elements of b point to the same place in memory because in JavaScript, the h==h operator on objects (including arrays) is defined to return true if and only if the objects\' ' ),
h('p', ' Here is what happens when harr.slice()h is pushed into b: ' ), 
h('pre', `  var arr = [1,2,3];
  var b = [arr.slice()];
  arr.push(4);
  b.push(arr.slice());
  ...
  console.log(b[1]);  // [1,2,3,4] ` ),
h('p', ' Success! harr.slice()h returned a copy of arr assigned to a new location in memory. It is anonymous, so the only way to access it is through b. b[0], b[1], and arr each have unique locations in memory so mutating one has no effect on the others. ' ), 
h('p', ' The sixteen-square grid below puts these concepts into practice. If you click two squares, they exchange locations on the grid. If you do this several times, the hBACKh and hFORWARDh buttons will display the result of each pair of clicks. ' ),
h('div#donkey', makeRDS( ArrDS[mMindexDS.x] )),
h('br'),
h('button#gridBack', 'back' ),
h('button#gridForward', 'forward' ),
h('p', hindex: h + mMindexDS.x  ),
h('p', hrNumsDS: h + rNumsDS.join(', ') ),
h('p', hrMatrixF(rNumsDS: h + rMatrixF(rNumsDS) ),
h('br'),
h('br'),
h('p', ' Reactivity is achieved in these demonstrations through Cycle.js rather than RxJS, Bacon, or something else that could work just as well, though perhaps not quite as elegantly. This section provides a glimpse of how I use (some might say, \hmisuse\h) Cycle.js. '),
h('p', ' If you click any two numbers above they will exchange places with one another. The array hrNumsDSh keeps track of the positions of numbers on the grid. ' ),
...
h('p', ' Then if you click the bottom right square (var m815), rADS expands to contain h3h and h15h. This causes rExchange(3,15) to run and return the updated value of rDataDS. ' ),
...
h('p', ' button#R9.mR has id h#R9h and formatting class hmRh. When numbers are rearranged, the id\'s remain in fixed positions. ' ),
...
h('p', ' This code undoubtedly deviates from Cycle.js and functional programming recommended practices. After all, rNumsDS and rDataDS are global variables tracking the state of the grid. Cycle.js favors maintaining state in streams and functional aficionados eschew global variables, period. Me, I just like to make things work as neatly and efficiently as I can. Making a state stream would be a hassle and, since this is not a group effort, nobody is going to hijack my variable names. So, for now, grid state will hang out in global scope. ' ),
  ...
  h('span#comments' ),
  h('img.image', {props: {src: herror2.pngh}}  ),
  h('div#comment'),
    h('h2', {style: {color: hredh }}, 'Comments' ),
...
h('p', ' If I were working in a group, or providing something that would be maintained by anyone other than me, I would code by the book. I\'m not getting paid for this work; I\'m just having fun. Dig it! '),
  ...
  h('pre', `else if hGZ#$42h \`T.isPrefixOf\` msg
                  -- FETCH AND BROADCAST ALL COMMENTS ON BROWSER LOAD
   then
       do
           st <- atomically $ readTVar state
           broadcast (hGZ#$42,h \`mappend\` group \`mappend\` h,h
             \`mappend\` sender \`mappend\` h,h \`mappend\` comments ) st

else if hGN#$42h \`T.isPrefixOf\` msg
   ...
   then
       do
           ...
           st <- atomically $ readTVar state
           broadcast (hGN#$42,h \`mappend\` group \`mappend\` h,h
               \`mappend\` sender \`mappend\` h,h \`mappend\` extra) st

 else if hGD#$42h \`T.isPrefixOf\` msg      -- DELETE A COMMENT
    then
        do
            ...
            st <- atomically $ readTVar state
            broadcast (hGD#$42,h \`mappend\` group \`mappend\` h,h
              \`mappend\` sender \`mappend\` h,h \`mappend\` extra) st

 else if hGE#$42h \`T.isPrefixOf\` msg      -- EDIT A COMMENT
    then
        do
            a <- TIO.readFile xcomments
            lk <- atomically L.new
            ...
            st <- atomically $ readTVar state
            broadcast (hGE#$42,h \`mappend\` group \`mappend\` com
  ...
  h('div#reactivity' ),
  h('br'),
  h('a', {props: {href: h#reactivity2h}}, 'Back to the pingpong demo' ),
  ...
  h('p', ' Whenever the stream incF$() is called, the stream returned by pingpongDriver is augmented and that causes main() to return an object whose only element is a map from the stream calcStream$ to a virtual DOM node with many children. calcStream$ is produced by merging 48 streams together including ping$ resulting from ping(n) where hnh is the number supplied by pingpongDriver (a/k/a hsources.PPh). The first two lines of main()\'s return value are shown below. ' ),
  ...
  h('pre', `  var pingpong$ = sources.DOM
    ...
    var ppR = {style: {color: 'red',
      marginLeft: '5%', fontSize: h26h}};
    var ppY = {style: {color: 'yellow',
      marginLeft: '42%', fontSize: h26h}};
...
h('p', ' The function diffRender() forces Snabbdom to perform one last diff and render. It is defined as follows: ' ),
  h('pre', `  const diffRender = () => document.getElementById(htestQh).click();`),
  h('p', ' The button htestQh cubes the number 2 in the MonadItter demonstration. It was chosen because it does not significantly affect anything aside from forcing Snabbdom to update the DOM. '),
  ...
  picked = this.s[0][this.s[1]][3].slice(),
  display = this.s[0][this.s[1]][4].slice()
  playerName = this.s[0][this.s[1]][5] ? this.s[0][this.s[1]][5].slice() : hnobodyh,
  playerGroup = this.s[0][this.s[1]][6] ? this.s[0][this.s[1]][6].slice() : hsoloh,
]) {
  this.s[1] += 1;
  var newState = this.s.slice();
  newState[0].splice(this.s[1], 0, [score, goals, operator, picked, display,playerName,playerGroup])
   console.log(h[score, goals, operator, picked, display]h,
...
function score(result) {
    ...
    if (scor === 25 && gameMonad.fetch1() === h2h) {  // The player wins.
        mMindex.ret(0);
 ...
 h('a', { props: { href: hhttps://github.com/dschalk/monads-in-JavaScripth,
   target: h_blankh } }, 'monads-in-JavaScript.'),
  h('p', '.'),
  h('p'),
...
h('a',   {props: {href: hhttps://github.com/getify/You-Dont-Know-JSh, target: h_blankh }},  'You Don\'t Know JavaScript by Kyle Simpson'),
...
h('a',   {props: {href: hhttps://github.com/getify/You-Dont-Know-JSh, target: h_blankh }},  'You Don\'t Know JavaScript'),
h('span', ' is the thinking developer\'s answer to ' ),
h('a',   {props: {href: hhttp://shop.oreilly.com/product/9780596517748.doh, target: h_blankh }},  ' hJavaScript: The Good Partsh by Douglas Crockford ' ),
h('p', ' That\'s not to say that Crockford isn\'t a thinking developer. He is a very bright guy. It\'s just that the so called hgood partsh are a dumbed down subset of JavaScript that help keep people out of trouble when they don\'t feel inclined understand the JavaScript programming language. ' ),
h('span#cyclet', ', which has long been revered as a hmust readh JavaScript book. Kyle Simpson recommends learning to use potentially dangerous code intelligently while Douglas Crockford advocates never using it at all. I think the phrase heval is evilh stems from Crockford\'s book. I find eval() to be very useful from time to time. Kyle Simpson teaches programmers how to safely tap the full potential of JavaScript. ' ),
h('p#defs'),
h('br'),
...
h('p', ' Demonstration 1 begins with the click of a button with id hfactors_Qh and ends with fifteen browser updates. The activity after each click is tied together by: ' ),
...
h('p', ' Clicking the button with id hfactors_Qh creates an event to which factorsClick7$ stream responds by prompting hbind(145)(x=>x*x*x)(it4)(it6)(it7)h to execute fifteen times. The factorsAction7$ stream flows into the virtual DOM where changes cause the Snabbdom diff and render process to update the browser. Here\'s some of the relevant code: ' ),
...
h('pre', `function createWebSocket(path) {
    ...
    var Socket = hMozWebSocketh in window ? MozWebSocket : WebSocket;
...
h('p', ' The prefix hxsh designates the name space for the xstream library. ' ),

h('pre', `function main(sources) {
  const messages$ = sources.WS.map( e => {
    var v = e.data.split(',')  // The string arriving from the server is made into an array.
    ...
    ret(v[0]  // hreth is defined below:
              // v[0] is the prefix of each message from the server.
    ...
    .bnd(next, 'BC#$42', mMZ28)  // Executes when a hBC$#42h-prefixed message arrives from the server
...
h('p', ' When anything prefixed by mMZ28 comes in from the server, mMZ28.release executes, causing mMZ40.bnd(r) to execute, where hrh is the random number obtained from the Haskell server. ' ),
...
h('p', ' hmMZ52.bnd(m => { ... h could have been placed anywhere. Putting it at the end of hbind(50)(x => x*x*x)(it4)(it6)(it7)hillustrates the flexibility of bind for the purposes of Example 1, but in production it would also make Demonstration 1 easier to understand by people responsible for maintaining the code. ' ),
...
h('p', ' After hbind(145)(cubeC)(it4)(it6)(it7) runs, the prime decomposition of the number it recieved arrives from workerG. The workerGDriver (sources.WWG) detects the message and adds it to the workerG$ stream. Here is the definition of workerGDriver:  ' ),
...
h('p', ' Clicking the hdecompose fifteen random numbersh button causes factorsRecursion(0) to execute. Here\'s the code: '),
...
h('a', {props: {href: '#top'}}, 'Back to the top'),
  h('br'),
h('h2', 'The >>= operator'),
h('p', ' The definition of >>= (pronounced hbindh) used below is a variation that uses monads. As you see, encapsulating values in monads works smoothly, but adds nothing of value. Useful monads are in the Haskell server on the backend of this web page. Here it is: ' ), 
h('pre', `operator >>= left 11 = (left, right) => {
  return #\`\${right}(\${left}.x)\`;
}; `),
h('span', ' >>= and expressions using >>= are compiled to JavaScript code with help from the ' ), 
h('a', {props: {href: hhttps://www.sweetjs.org/h}}, 'sweet.js library'),
h('span', '.' ),
...
h('span', 'JavaScript' ),
h('pre', {style: {color: h#FFD700h, fontSize: h15h}}, `function Monad2(z_146) {
  this.x = z_146;
} 
...
h('a', {props: {href: h#haskellh}}, 'above' ), 
...
h('span', ' As discussed ' ),
h('a', {props: {href: h#haskellh}}, 'below' ), 
